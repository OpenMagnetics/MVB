...
class TestWinding(unittest.TestCase):
    ...
    def test_round_wire_winding_cadquery(self):
        ...
        bobbin_dims = {"width": 0.008, "height": 0.015}
    ...
    def test_winding_ccw_direction(self):
        ...
        bobbin_dims = {"width": 0.006, "height": 0.012}

        b = builder.Builder("CadQuery")
        result = b.get_winding(winding_data, bobbin_dims, name="test_winding_ccw", output_path=self.output_path)
    ...
    def test_bulk_winding_high_turn_count(self):
        ...
        bobbin_dims = {"width": 0.01, "height": 0.02}

        b = builder.Builder("CadQuery")
        result = b.get_winding(winding_data, bobbin_dims, name="test_winding_bulk", output_path=self.output_path)

        self.assertIsNotNone(result)

    def test_winding_no_export(self):
        """Test winding generation returning geometry object"""
        winding_data = {"name": "test_winding", "type": "round_wire", "wireDiameter": 0.0005, "insulationThickness": 0.00005, "numberOfTurns": 5, "numberOfLayers": 1, "windingDirection": "cw"}

        bobbin_dims = {"width": 0.008, "height": 0.015}

        b = builder.Builder("CadQuery")
        result = b.get_winding(winding_data, bobbin_dims, name="test_winding_geom", output_path=self.output_path, export_files=False)

        self.assertIsNotNone(result)

    def test_single_layer_winding(self):
        """Test single layer winding"""
        winding_data = {"name": "single_layer", "type": "round_wire", "wireDiameter": 0.001, "insulationThickness": 0.0001, "numberOfTurns": 8, "numberOfLayers": 1, "windingDirection": "cw"}

        bobbin_dims = {"width": 0.01, "height": 0.015}

        b = builder.Builder("CadQuery")
        result = b.get_winding(winding_data, bobbin_dims, name="test_winding_single_layer", output_path=self.output_path)
    ...
    def test_winding_from_mas_turnsDescription(self):
        ...
        bobbin_dims = {"width": 0.008, "height": 0.033}

        b = builder.Builder("CadQuery")
        result = b.get_winding(winding_data, bobbin_dims, name="test_winding_mas", output_path=self.output_path)
    ...
    def test_winding_fallback_without_turnsDescription(self):
        ...
        b = builder.Builder("CadQuery")
        result = b.get_winding(winding_data, bobbin_dims, name="test_winding_fallback", output_path=self.output_path)
